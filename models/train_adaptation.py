...
from adaptive_part import AdaptiveEncoder
...
adapt_model = AdaptiveEncoder(30, 3, 5)
model = LatentDynamicsModel(50, 3, 5)
model.load_state_dict("Dynamics.pt")
model.eval()

# Set all layers to frozen
...
for epoch_i in pbar:
    ...
    for batch_i, data in enumerate(train_loader):
      ...
      adapt_pairs = data['adapts']
      #R_img = data['R_img']

      #reconstructed_states, mu, log_var, latent_state = vae_model(states)
      adapt = adapt_model(adapt_pairs)
      results = model(imgs, actions, adapt)
...
torch.save(adapt_model.state_dict(), 'AdaptModel.pt')